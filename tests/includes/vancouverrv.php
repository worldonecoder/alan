		<article id="vancouverrv" class="view container">
			<div class="row project-row">

				<div class="item col-sm-8">
					 <a class="project-feature-img" href="http://www.vancouverrv.com/" target="_blank">
					 	<img id="img-vancouverrv" class="int-img" src="images/vancouverrv.jpg" alt="Siamkas Car Company" />
					 </a>
				</div>

				<div class="item col-sm-4 project">
					<h1 class="project-title">VANCOUVER RV</h1>
					<p class="project-role">WEB DESIGN</p>
					<div class="project details">
						<p>Malis voluptaria eu usu, probo nominavi scriptorem duo no. Iudico labores appareat sed ut. Facete omittam accusata cu mei, idque nonumy nominavi qui et. Adhuc iudicabit has in, eripuit abhorreant vel in.</p>
						<p>Facete omittam accusata.</p>
						<div class="project-buttons">
							<a class="go-to-site page-button" href="http://www.hillyersstaytonford.com/" target="_blank">VIEW WEBSITE</a>
							<a class="contact-alan page-button" href="javascript:void(0);">CONTACT ALAN</a>
							<div class="prev-next-container">
								<a class="prev-project page-button transitioner" href="javascript:void(0);"><i class="fa fa-caret-left"></i>PREV</a>
								<a class="next-project page-button transitioner" href="javascript:void(0);">NEXT <i class="fa fa-caret-right"></i></a>
							</div>
						</div>
					</div>
				</div>

			</div>
		</article>